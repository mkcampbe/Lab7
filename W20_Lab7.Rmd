...
knitr::opts_knit$set(root.dir = "~/Box Sync/Schmidt_Lab/Bio201_teaching/W20_Protocols/Lab7/")
```
...
Create a stacked bar plot for participants who consumed BRMPS twice a day. Use facets two create a panel for each study week. 
```{r}
...
wilcox.test(rich_df_wide$week1, rich_df_wide$week3, 
            alternative = "two.sided", paired = TRUE) # p-value = 0.02531
```
...
Create a plot that shows the richness across weeks 1 and 3 in participants who consumed BRMPS once and twice a day.
```{r}

```

Conduct statistical tests to determine the if the richness was significantly different in participants who consumed BRMPS twice a day. 
```{r}
