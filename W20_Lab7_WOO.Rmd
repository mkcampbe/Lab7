...
```{r}
shared_m <- read_delim("raw_data/Lab7_data/MMP_thruf19_filtered.shared",
...
```{r}
taxa_m <- read_delim("raw_data//Lab7_data/MMP_thruf19.taxonomy",
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, na=c("NA")) %>%
...
```{r}
samples_m <- read_delim("raw_data/Lab7_data/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
...
```{r}
# format sample data 
samples_df <- read_delim("raw_data/Lab7_data/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
                            col_types = cols(Sample_number = col_character(),
                                             Bristol_numeric = col_number(),
...
```{r}
brmps_check<- plot_grid(brm2_plot, brm1_plot,
          nrow = 1, ncol = 2)
brmps_check #colors for types of bacteria arent matching 

phylum_colors_2 <- c("#00767d", "#d38665", "#0d8462", "#C2EFEB", "#d68ab9","#31745c",
                    "#8564a4","#a6a55d",
                   "#907030")
phylum_colors_1 <- c("#00767d", "#d38665", "#0d8462", "#d68ab9",
                   "#31745c", "#8564a4", "#a6a55d", "#975657",
                   "#656c4c", "#907030")
...
brmps2_plot_new <- brm2_plot + 
  scale_fill_manual(values = phylum_colors_2) +
  theme(legend.position = "none")

brmps1_plot_new <- brm1_plot + 
  scale_fill_manual(values = phylum_colors_1) +
  ylab(NULL) +
  theme(legend.position = "none")

brmps_barplot <- plot_grid(brmps2_plot_new, brmps1_plot_new,
                           nrow = 1, ncol = 2,
                           rel_widths = c(1.5, 3.5, 1))
brmps_barplot

save_plot(brmps_barplot,
          base_width = 12, base_height = 4,
          filename = "figures/brmps_barplot.pdf")
```
...
brmps_plot <- physq_phyla %>%
  ...
  ggplot(aes(x = participant_id, y = relative_abundance,
             ...
             study_week, frequency)) + 
  geom_bar(stat = "identity") +
  facet_grid(~study_week, scales = "free") +
  theme(axis.title.x = element_blank(),
        axis.text.x =  element_text(angle = 90,
                                    vjust = 0.5,
                                    hjust = 0.5,
                                    size = 8)) +
  ylab("Relative Abundance\n(% total sequences)") +
  ggtitle("Week 1 vs Week 3 - BRMPS 2xdaily")
brmps_plot
```

# Alpha diversity

There are numerous alpha diversity metrics, however we are going to focus on observed (richness) and Simpson (evenness) for our analyses. We will calculate, plot, extract information about alpha diversity with the plot_richness() function. 

### Calculate richness
Import sample data, format and filter data frame for samples in which we want to investigate the richness. 
```{r}
samp_data <- read_delim("raw_data/Lab7_data/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
                            col_types = cols(Sample_number = col_character(),
                                             Bristol_numeric = col_number(),
...
Did you conclude the richness changed from week 1 to week 3 in individuals who consumped LOODAT? the richness did change from week 1 to week 3 because our p-value of 0.02531 which is less than our significance level. Additionally, the widest part of the violin plots of the average richness value between week 1 and week 3 appear to be different.  

### Practice 3

Create a plot that shows the richness across weeks 1 and 3 in participants who consumed BRMPS once and twice a day. Hint: use facets. facet = facet_grid(~frequency, scales = "free_x") 
way to further subset data and still end up with only one figure. could also facet_grid(~study_week, scale)
```{r}
...
```{r}
# use ordinate function on subset *** play around with the numbers to see how it affects the output 
...
physq_bc
```
want stress to be less than 0.2. 
...
```{r}
# ordinate
physq_j <- ordinate(physq_scale, 
           method = "NMDS", 
           k=3, maxit=500, try=50,
           distance = "jaccard")  # switch this to distance = "jaccard" for jaccard score 
physq_j
```
...
# start with same phyloseq object as above 
# calculate BC index, get distance matrix
dat_jac <- phyloseq::distance(physq_sub, method = "jaccard") 
#change method = "jaccard"
sampledf2 <- physq_sub %>% 
  sample_data(.) %>% #extract sample data from phyloseq object 
  as(., "data.frame") #convert to data frame for adonis()
...
# run test
adn_res2 <- adonis(formula = dat_jac ~ study_week, 
                  data = sampledf2)
